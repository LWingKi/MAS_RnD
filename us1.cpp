...
{   
    ...
    double k_p_position_angular = 80;
    ...
    // double k_p_position_linear = 10;
    // double k_p_position_angular = 15;
    // double k_p_velocity_linear = 10;
    // double k_p_velocity_angular = 8;
    // double k_i_position_linear = 0.5;
    // double k_i_position_angular = 0.2;
    // double k_i_velocity_linear = 0.9;
    // double k_i_velocity_angular = 0.9;
    ...
    GNUPlotter plot_graph(path.parent_path()/"log/us1/contactless2",false,false);
    ...
    *eff_cmd = *eff_msr * -1;
    ...
    fksolver.JntToCart(q, initialPos);
    goalPos.p = initialPos.p + KDL::Vector(0.0,0,-0.25);
    goalPos.M = initialPos.M;
    ...
    do{
        ...
        accumulate_velocity.push(currVel.vel.z());
            ...
            if (accumulate_velocity.size() == sampling_size){
                norm_velocity = sum_velocity/sampling_size;
                accumulate_velocity.pop(); // Pop the first element in the queue
                // condition to switch the state
                if (abs(norm_velocity) < 0.0001){
    ...
    KDL::Vector force2(-10.0, 0.0, 0.0);
    ...
    // k_p_position_linear = 15;
    // k_p_position_angular = 15;
    // k_p_position_linear = 10;
    // k_p_position_angular = 90;
    // k_p_velocity_linear = 10;
    // k_p_velocity_angular = 40;
    k_p_position_linear = 10;
    k_p_position_angular = 80;
    k_p_velocity_linear = 4;
    k_p_velocity_angular = 40;
    ...
    do{
        ...
        if (vel_set_point.vel.x() > velocity_limit){
            vel_set_point.vel.data[2] = velocity_limit;
        }
        if (vel_set_point.vel.x() < -velocity_limit){
            vel_set_point.vel.data[2] = -velocity_limit;
        ...
        robif2b_kinova_gen3_update(&rob);
        counter_2++;
        // logger.logInfo("pos_error: %s",pos_error);
        // logger.logInfo("currPos:%s",currPos.p);
        // logger.logInfo("goalPos:%s",goalPos.p);
        // logger.logInfo("beta:%s",beta);
        // logger.logInfo("constrain_torque:%s\n",constrain_torque);
    // }while(abs(pos_error.vel.x()) > 0.001);
    }while(true);
